...
type ViewMode = "lesson" | "exam" | "glossary" | "flashcards" | "certificate" | "capstone" | "diagnostic" | "pm-coach" | "notebook" | "interview" | "templates" | "analytics" | "data-exercises" | "portfolio" | "resume-review" | "competency-radar" | "admin";
...
export default function App() {
  ...
  const [appStep, setAppStep] = useState<"loading" | "auth" | "onboarding" | "pricing" | "welcome" | "course">("loading");
  ...
  const loadAccessLevel = useCallback(async (accessToken: string, userId: string) => {
    ...
    if (isAdminMode) {
      console.log(`[App] 🛡️ Admin mode active — skipping loadAccessLevel reset`);
      return;
    }
  ...
  const numberToModuleIdMap = useMemo<Record<number, string>>(() => {
    ...
    console.log(`[🗺️ numberToModuleIdMap] Построен маппинг дл�� ${courseModules.length} модулей:`, map);
  ...
  const handleAdminAuthenticated = useCallback((pw: string) => {
    console.log(`[App] 🛡️ Admin authenticated — granting full course access`);
    setIsAdminMode(true);
    setAdminPassword(pw);
  ...
  const handleSelectLesson = useCallback((lessonId: string) => {
    ...
    if (!canAccessPaidContent && !isAdminMode && (!isFreeOrBonus || isPaidOnly) && !TESTING_ALL_OPEN) {
      const allLessons = getAllLessons();
      const lessonData = allLessons.find(l => l.lesson.id === lessonId);
    ...
    if (lessonData) {
      ...
      if (isFirstLesson) {
        ...
        const alreadyIntroduced = mod.id !== "m18" && !!localStorage.getItem(`introduced-module-${mod.id}`);
  ...
  const renderMainContent = () => {
    ...
    switch (viewMode) {
      case "admin":
        return (
          <AdminPanel
            ...
            onClose={() => { setViewMode("lesson"); setSelectedLesson(prev => prev || "m1-l1"); }}
          />
        );
      case "exam":
      ...
      default:
        ...
        if (!canAccessPaidContent && !isAdminMode && selectedLesson && (!FREE_LESSON_IDS.has(selectedLesson) || PAID_ONLY_LESSON_IDS.has(selectedLesson)) && (!bonusLessons.has(selectedLesson) || PAID_ONLY_LESSON_IDS.has(selectedLesson)) && !TESTING_ALL_OPEN) {
          return (
            <PaywallScreen
  ...
  if (appStep === "auth") {
    ...
    if (authMode === "landing") {
      return (
        <>
          <CourseLanding
            onStart={() => { setAppStep("onboarding"); window.scrollTo(0, 0); }}
            onLogin={() => { setAuthMode("login"); window.scrollTo(0, 0); }}
          />
          <AnimatePresence>
            {showAdminPanel && (
              <AdminPanel onClose={() => setShowAdminPanel(false)} onAuthenticated={handleAdminAuthenticated} />
            )}
          </AnimatePresence>
        </>
      );
    ...
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      {isDemoMode && !canAccessPaidContent && (
        ...
        >
          ...
          <button
            onClick={() => { setAppStep("pricing"); window.scrollTo(0, 0); }}
      ...
      <main className="flex-1 min-w-0 overflow-hidden relative flex flex-col">
        ...
        {isAdminMode && (
          <div className="shrink-0 flex items-center gap-1 px-3 py-1.5 border-b border-border bg-card/80 backdrop-blur-sm z-10">
            ...
            <button
              onClick={() => { setViewMode("lesson"); setSelectedLesson(prev => prev || "m1-l1"); }}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                viewMode !== "admin"
        ...
        <AnimatePresence mode="wait" initial={false}>
          <motion.div
            ...
            className={isAdminMode ? "flex-1 min-h-0 overflow-hidden" : "h-full"}
          >
            {renderMainContent()}
          </motion.div>
        </AnimatePresence>
