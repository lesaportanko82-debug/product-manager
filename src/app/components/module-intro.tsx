...
import { courseModules, type Module } from "./course-data";
import { YaroslavAuthorCard } from "./yaroslav-author-card";
import yaroslavCardImg from "../../imports/ChatGPT_Image_6_____2026__.__14_48_18_1.png";
import yaroslavCardMobileImg from "../../imports/ChatGPT_Image_6_____2026__.__16_48_25_1.png";
...
export function ModuleIntroScreen({ module, onStart, lessonCount }: ModuleIntroProps) {
  ...
  return (
    <div className="flex-1 h-full overflow-y-auto bg-gradient-to-br from-slate-50 via-white to-teal-50/30 dark:from-slate-950 dark:via-slate-900 dark:to-teal-950/20 flex flex-col items-center p-4 py-10">
      <div className="w-full max-w-lg">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-teal-100 dark:bg-teal-900/40 border border-teal-200/60 dark:border-teal-700/40 text-teal-700 dark:text-teal-300 text-sm font-semibold">
            Модуль {courseModules.findIndex(m => m.id === module.id) + 1} из {courseModules.length}
          </div>
        </motion.div>
        ...
        {/* Author card — only for m18 (UI/UX by Yaroslav Shuvaev) */}
        {module.id === "m18" && (
          <motion.div
            ...
            className="mt-5"
          >
            {/* Desktop: show the ready-made banner image */}
            <img
              src={yaroslavCardImg}
              alt="Ярослав Шуваев — партнёрский модуль"
              className="hidden sm:block w-full rounded-2xl shadow-2xl"
            />
            {/* Mobile: show the mobile banner image */}
            <img
              src={yaroslavCardMobileImg}
              alt="Ярослав Шуваев — партнёрский модуль"
              className="block sm:hidden w-full rounded-2xl shadow-2xl"
            />
          </motion.div>
        )}
